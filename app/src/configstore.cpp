...
bool ConfigStore::saveConfig(const QVariantMap &config) const
{
    ...
    // Merge over what is already on disk instead of replacing it. Setup writes
    // only the display keys, so a plain truncate silently dropped the
    // resource-control ones (CLOSE_APPS_ENABLED, CLOSE_APPS_WAIT_SECONDS,
    // APPS_TO_CLOSE) every time the user saved a display setting. The engine
    // reads this file with `source`, so a key is only ever set, never deleted.
    QVariantMap merged = loadConfig();
